...
import type { InventoryItem, TicketPriority } from "@/lib/models/types"
import { Loader2, ArrowLeft, CheckCircle2 } from "lucide-react"
import Link from "next/link"
import { BarcodeScanner } from "@/components/barcode/barcode-scanner"

const categories = ["Hardware Issue", "Software Issue", "Network Issue", "Account/Access", "Email", "Printer", "Other"]
...
export function CreateTicketForm({ availableItems }: CreateTicketFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (itemId) {
      const item = scannedItem || availableItems.find((i) => i._id?.toString() === itemId)
      if (item) {
        itemInfo = {
          itemId,
    ...
    startTransition(async () => {
      const result = await createTicket({
        ...
        reportedBy: {
          ...
          department: formData.get("reporterDepartment") as string,
        },
        ...itemInfo,
      })
      ...
      <Card className="max-w-2xl mx-auto">
        <CardContent className="pt-6">
          <div className="text-center space-y-6">
            ...
            <div>
              <h2 className="text-xl font-semibold">Ticket Created Successfully</h2>
              <p className="text-muted-foreground mt-1">Your ticket number is:</p>
              <p className="text-2xl font-mono font-bold mt-2">{createdTicketNumber}</p>
            </div>
            <div className="flex gap-4 justify-center">
              ...
              <Button onClick={() => router.push("/tickets")}>View All Tickets</Button>
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="reporterName" name="reporterName" required className="bg-secondary" placeholder="John Doe" />
              </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <BarcodeScanner onScan={handleBarcodeScanned} placeholder="Scan item for repair..." />
              </div>

              {scannedItem && (
                <div className="p-4 rounded-lg bg-info/10 border border-info/30">
                  <p className="font-medium">{scannedItem.name}</p>
                  <p className="text-sm text-muted-foreground">{scannedItem.barcode}</p>
                </div>
              )}
